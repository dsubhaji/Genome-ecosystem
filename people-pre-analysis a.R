...
repeat
{
  ...
  if (choice==0)
  {
    pOverview = fetch(dbSendQuery(mydb, "select ;"))  
    write.csv(file="people-pre-analysis/pView Data in csv/poverview.csv", x=pOverview)
  ...
  if (choice==1)
  {
    NumTimeCommitter = fetch(dbSendQuery(mydb, "select p.id as 'people_id', count(s.id) as 'pCommitter', p.email from scmlog s join people p on s.committer_id=p.id group by s.committer_id;"))
  ...
  if (choice==2)
  {
    NumTimeAuthors  = fetch(dbSendQuery(mydb, "select p.id as 'people_id', count(s.id) as 'pAuthor', p.email from scmlog s join people p on s.author_id=p.id group by s.author_id;"))
