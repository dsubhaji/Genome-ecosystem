	...
	public static void main(String[] args) throws IOException
	{
       ...
       for(int i=0; i < size; i++)
	    ...
	    String[] data=(pList.get(i)).split(",");
	   	if(idstatus!=Integer.parseInt(data[0])) //Skip same people_id eg (2 2)
	    {
	   		vert++;
	   		idstatus = Integer.parseInt(data[0]); //identify people_id
	    }
 		}
       ...
       for(int i=0; i < size; i++)
      	{
    	   ...
    	   String[] data=(pList.get(i)).split(",");
    	    if(idstatus!=Integer.parseInt(data[0]))
    	    {
    	...
    	int common=0;	
       			...
       			for(int b=0; b<states.length; b++)
       			{
	       			...
	       			for(int c=0; c<count;c++) // store repoid 1
	       			{
	       				...
	       				if(Integer.parseInt(data[0])==states[b])   // data[0] is 1, store repoid of all peopleid 1
	       				{
	       					repo[c]=Integer.parseInt(data[1]);
	       			...
	       			for(int d=0; d<states.length; d++)
	       			{
	       				if(states[d]!=vert1)
	       				{
			       			...
			       			{
			       				String[] data=(pList.get(c)).split(",");
			       				if(Integer.parseInt(data[0])==states[d]) 
			       				{
			       					repo2[c]=Integer.parseInt(data[1]);  
			       				}	
			       			}
